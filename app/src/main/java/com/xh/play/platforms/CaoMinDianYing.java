...
public class CaoMinDianYing extends AbsPlatform {
    private static final String HOST = "http://www.caominyy5.com";
//    https://www.lovedan.net/vodplay/117679-1-1.html

    @Override
    public List<Title> types() {
        String url = "http://www.caominyy5.com/";
    ...
    public boolean playDetail(Detial detial) {
        ...
        try {
            ...
            List<JXNode> divs = jx.selN("//div[@class='row clearfix']/div");
            for (JXNode div : divs) {
                String text = "";
                List<JXNode> nodes = div.sel("div/text()");
                if (nodes.size() > 0)
                ...
                for (JXNode node : nodes) {
                    ...
                    playUrl.title = text + " " + element.text();
    ...
    public List<Detial> search(String text) {
        String url = String.format("http://www.caominyy5.com/vodsearch/-------------.html?wd=%s",text);
